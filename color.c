...
/*   Updated: 2020/10/06 11:49:15 by anikkane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_ray		new_hit_direction(t_data *data, t_ray org_ray, t_vector *n)
{
	...
	ray.newstart = org_ray.newstart;
	ray.start = org_ray.start;
	ray.target = org_ray.target;
	...
	while (i >= 0)
	{
		if (data->iter == data->org_iter)
		{
			if (shadow(data, ray.newstart, i) != 1)
				rgb = get_light(data, rgb, ray.newstart, n, i);
		}
		else
			rgb = get_light(data, rgb, ray.newstart, n, i);
		i--;
